...
export type ActionStatus = 'open' | 'assigned' | 'in_progress' | 'completed' | 'verified' | 'rejected' | 'overdue'
...
export interface Patrol {
  ...
  likelihood: 1 | 2 | 3 | 4
  severity: 1 | 2 | 3 | 4
  risk_score?: number // Optional - calculated but may not be stored
  risk_level: RiskLevel
  recommended_action: RecommendedAction
...
export interface PatrolPhoto {
  ...
  caption?: string
  photo_data?: string // Base64 data for offline
  uploaded_by?: string
  taken_by?: string
  created_at?: string
}
