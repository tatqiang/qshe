...
onMounted(() => {
  if (props.action) {
    ...
    if (props.action.verification_photos && props.action.verification_photos.length > 0) {
      formData.value.verificationPhotos = props.action.verification_photos.map((photo: any) => ({
...
const getUserInitial = () => {
  ...
  if (currentUser && typeof currentUser === 'object' && 'name' in currentUser && typeof currentUser.name === 'string') {
    return currentUser.name.charAt(0).toUpperCase()
  }
  if (currentUser && typeof currentUser === 'object' && 'email' in currentUser && typeof currentUser.email === 'string') {
...
const handleApprove = async () => {
  ...
  try {
    ...
    const newPhotos = formData.value.verificationPhotos.filter((p: any) => !p.existing).map((p: any) => p.file)
