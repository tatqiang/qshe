        ...
        <div class="mb-3 sm:mb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4">
            <div>
              ...
              <span class="text-sm sm:text-base text-gray-900 dark:text-white font-medium">{{ [patrol.main_area, patrol.sub_area1, patrol.sub_area2].filter(Boolean).join(' › ') || 'No location specified' }}</span>
            </div>
            <div>
...
const showVerificationForm = ref({}) // Use an object to track form visibility per action
...
const patrolId = route.params.id

const isPatrolCreator = computed(() => {
  return patrol.value && user.value && patrol.value.created_by === user.value.id
})
...
const canEditVerification = (action) => {
  if (!user.value || user.value.id !== action.verified_by) {
    return false;
  }
...
const handleEditVerification = (action) => {
  showVerificationForm.value[action.id] = true;
};

const getPatrolTypeDisplay = (type) => {
  const typeMap = {
...
const fetchPatrolDetails = async () => {
  try {
    ...
    photos.value = photoData.map(p => ({ ...p, type: 'patrol' }))

    const actionData = await patrolService.getActions(patrolId)
    correctiveActions.value = actionData.map(action => {
      // Initialize verification form state for each action
      showVerificationForm.value[action.id] = false
      return {
        ...action,
        verification_photos: action.verification_photos.map(p => ({ ...p, type: 'verification' })),
        photos: action.photos.map(p => ({ ...p, type: 'action' }))
      }
    })
...
const handleStartVerification = (action) => {
  showVerificationForm.value[action.id] = true
}

const handleCancelVerification = (action) => {
  showVerificationForm.value[action.id] = false
}

const handleCorrectiveActionSubmit = async (formData) => {
  try {
    loading.value = true
    
    console.log('🔍 handleCorrectiveActionSubmit - formData:', formData)
    console.log('🔍 handleCorrectiveActionSubmit - user.value:', user.value)
    console.log('🔍 handleCorrectiveActionSubmit - user.value?.id:', user.value?.id)
    ...
    if (formData.id) {
      ...
      if (photos && photos.length > 0) {
        ...
        await patrolService.uploadActionPhotos(patrol.value.id, id, photos, createdBy)
      }
    } else {
      // Create new action
      ...
      const newAction = await patrolService.createAction(patrol.value.id, formData, userId)
...
const handleVerificationApprove = async (action, verificationData) => {
  try {
    loading.value = true
    ...
    console.log('🔍 Verification Approve - User ID:', userId)
...
const handleVerificationReject = async (action, verificationData) => {
  try {
    loading.value = true
    ...
    console.log('🔍 Verification Reject - User ID:', userId)
...
const handlePatrolPhotoClick = (index) => {
  openPhotoViewer(photos.value, index)
}

const handleActionPhotoClick = (actionPhotos, index) => {
  openPhotoViewer(actionPhotos, index)
}

const handleVerificationPhotoClick = (verificationPhotos, index) => {
