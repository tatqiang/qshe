...
const selectedMainAreaId = ref(null)
const selectedSubArea1Id = ref(null)
const selectedSubArea2Id = ref(null)
...
const availableRiskCategories = ref([])
...
const formData = ref({
  ...
  sub_area_2: '',
  specific_location: '',
  risk_categories: [], // Array of category IDs
  risk_items: [], // Array of item IDs
  likelihood: null,
  severity: null,
  remark: '',
...
const getRiskCellClass = (likelihood, severity) => {
...
const selectRisk = (likelihood, severity) => {
...
const loadRiskCategories = async () => {
  try {
    ...
    availableRiskCategories.value = (data || []).map(cat => ({
      ...cat,
...
const loadPatrol = async () => {
  if (props.mode === 'edit' && props.patrolId) {
    isLoadingPatrol.value = true
    try {
      const patrol = await patrolService.getById(props.patrolId)
      ...
      if (patrol) {
        ...
        console.log('🔍 DEBUG: Area values from DB:', {
          main_area: patrol.main_area,
          sub_area1: patrol.sub_area1,
          sub_area2: patrol.sub_area2,
        ...
        // If we have text values but missing IDs, look them up
        // This handles legacy data where IDs weren't saved
        if (patrol.sub_area1 && !patrol.sub_area_1_id && patrol.main_area_id) {
          const { data: subArea1 } = await supabase
            .from('sub_areas_1')
            .select('id')
            .eq('main_area_id', patrol.main_area_id)
            .eq('sub_area_1_name', patrol.sub_area1)
            .single()
          if (subArea1) {
            selectedSubArea1Id.value = subArea1.id
            console.log('🔍 DEBUG: Found sub_area_1_id:', subArea1.id)
          }
        }
        
        if (patrol.sub_area2 && !patrol.sub_area_2_id && selectedSubArea1Id.value) {
          console.log('🔍 DEBUG: Looking up sub_area_2_id for:', {
            sub_area_2_name: patrol.sub_area2,
            sub_area_1_id: selectedSubArea1Id.value
          })
          
          const { data: subArea2, error: lookupError } = await supabase
            .from('sub_areas_2')
            .select('id')
            .eq('sub_area_1_id', selectedSubArea1Id.value)
            .eq('sub_area_2_name', patrol.sub_area2)
            .single()
          
          if (lookupError) {
            console.error('❌ Error looking up sub_area_2_id:', lookupError)
          }
          
          if (subArea2) {
            selectedSubArea2Id.value = subArea2.id
            console.log('🔍 DEBUG: Found sub_area_2_id:', subArea2.id)
          } else {
            console.warn('⚠️ Could not find sub_area_2_id for:', patrol.sub_area2)
          }
        }
        ...
        if (props.mode === 'edit') {
          try {
            const photos = await patrolService.getPhotos(props.patrolId)
        ...
        formData.value = {
          ...
          main_area: patrol.main_area || '',
          sub_area_1: patrol.sub_area1 || '',
          sub_area_2: patrol.sub_area2 || '',
          specific_location: patrol.specific_location || '',
          risk_categories: categoryLinks?.map(c => c.risk_category_id) || [],
          risk_items: itemLinks?.map(i => i.risk_item_id) || [],
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let patrol
    if (props.mode === 'edit') {
      patrol = await patrolService.update(props.patrolId, patrolData)
      
      // Update risk categories junction table
      await supabase.from('patrol_risk_categories').delete().eq('patrol_id', props.patrolId)
      if (formData.value.risk_categories.length > 0) {
        await supabase.from('patrol_risk_categories').insert(
          formData.value.risk_categories.map(cat_id => ({
            patrol_id: props.patrolId,
      ...
      await supabase.from('patrol_risk_items').delete().eq('patrol_id', props.patrolId)
      if (formData.value.risk_items.length > 0) {
        await supabase.from('patrol_risk_items').insert(
          formData.value.risk_items.map(item_id => ({
            patrol_id: props.patrolId,
      ...
      const newPhotos = formData.value.photos.filter(p => !p.existing)
      if (newPhotos.length > 0) {
        await uploadPatrolPhotos(props.patrolId, newPhotos)
      }
    ...
    if (props.mode === 'edit') {
      ...
      router.push(`/patrol/${props.patrolId}`)
    } else {
...
const uploadPatrolPhotos = async (patrolId, photos) => {
