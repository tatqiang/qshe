          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(80vh-8rem)]">
              ...
              <div
                ...
                :class="!projectStore.selectedProject 
                  ? 'border-[#6FB3B8] bg-[#C2EDCE]/20 dark:bg-[#6FB3B8]/20' 
              ...
              >
                ...
                <div v-if="!projectStore.selectedProject" class="flex items-center justify-center w-6 h-6 rounded-full bg-[#6FB3B8]">
              ...
              <div v-if="projectStore.loading" class="flex items-center justify-center py-8">
                <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-[#6FB3B8]"></div>
                <span class="ml-2 text-gray-600 dark:text-gray-400">Loading projects...</span>
              </div>

              <!-- Projects List -->
              <div v-else-if="projectStore.projects.length > 0" class="space-y-3">
                <div
                  v-for="project in projectStore.projects"
                  :key="project.id"
                  @click="selectProject(project)"
                  class="flex items-center justify-between p-4 rounded-lg border-2 cursor-pointer transition-all"
                  :class="projectStore.selectedProject?.id === project.id 
                    ? 'border-[#6FB3B8] bg-[#C2EDCE]/20 dark:bg-[#6FB3B8]/20' 
                    : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'"
                >
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-1">
                      ...
                      <span
                        v-if="project.is_test_project"
                        class="px-2 py-0.5 text-xs font-medium rounded-full bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400"
                      >
                        Test
                      </span>
                    </div>
                  ...
                  <div v-if="projectStore.selectedProject?.id === project.id" class="flex items-center justify-center w-6 h-6 rounded-full bg-[#6FB3B8] ml-3">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                  </div>
