    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="font-medium text-blue-900 flex items-center text-lg">
          ...
          {(() => {
            ...
            return (
              <button
                type="button"
                className={`px-4 py-2 border text-sm rounded flex items-center space-x-2 font-medium ${
                  isDisabled 
        ...
        <div className="mb-6">
          <h4 className="font-medium text-blue-800 mb-4 text-base">📋 Patrol Information</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <span className="text-sm font-medium text-gray-700">Patrol Type: </span>
              <span className="text-base text-gray-900 font-medium">{getPatrolTypeDisplay(patrol.patrolType || 'scheduled')}</span>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-700">Patrol Date: </span>
              <span className="text-base text-gray-900 font-medium">{patrol.createdAt ? new Date(patrol.createdAt).toLocaleDateString() : 'N/A'}</span>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="mb-3">
              ...
              <div className="mt-1 bg-white px-3 py-2 rounded border text-base font-medium text-gray-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <span className="text-base text-gray-900 font-medium">{patrol.location || 'No location specified'}</span>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-700">Specific Location: </span>
              <span className="text-base text-gray-900 font-medium">{patrol.specificLocation || 'Not specified'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 pt-4 border-t border-blue-200">
          <h4 className="font-medium text-blue-800 mb-4 text-base">⚠️ Risk Assessment</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
              <span className="text-sm font-medium text-gray-700">Risk Level: </span>
              <span className={`inline-flex px-2 py-1 text-sm font-medium rounded-full ${riskInfo.color}`}>
                {riskInfo.priority} {riskInfo.level}
              </span>
              <div className="text-sm text-gray-600 mt-1">Score: {(patrol.likelihood || 1) * (patrol.severity || 1)}</div>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-700">Likelihood: </span>
              <span className="text-base text-gray-900 font-medium">Level {patrol.likelihood || 1} of 5</span>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-700">Severity: </span>
              <span className="text-base text-gray-900 font-medium">Level {patrol.severity || 1} of 5</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <span className="text-sm font-medium text-gray-700">Immediate Hazard: </span>
        ...
        {(selectedCategories.length > 0 || selectedItems.length > 0) && (
          <div className="mb-6 pt-4 border-t border-blue-200">
            <h4 className="font-medium text-blue-800 mb-4 text-base">📋 Risk Categories & Items</h4>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {selectedCategories.map((category) => (
                    <span key={category.id} className="inline-flex px-2 py-1 text-sm font-medium bg-blue-100 text-blue-800 rounded">
                      {category.name}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {selectedItems.map((item) => (
                    <span key={item.id} className="inline-flex px-2 py-1 text-sm font-medium bg-purple-100 text-purple-800 rounded">
                      {item.name}
                    </span>
                  ))}
                </div>
        ...
        {photos.length > 0 && (
          <div className="pt-4 border-t border-blue-200">
            <h4 className="font-medium text-blue-800 mb-4 text-base">📸 Evidence Photos</h4>
        ...
        {/* Remark Section */}
        <div className="pt-4 border-t border-blue-200">
          <h4 className="font-medium text-blue-800 mb-3 text-base">💬 Remark</h4>
          <div className="bg-gray-50 rounded-lg p-4">
            {patrol.remark && patrol.remark.trim() ? (
              <p className="text-gray-700 whitespace-pre-wrap">{patrol.remark}</p>
            ) : (
              <p className="text-gray-500 italic">No remark provided</p>
            )}
          </div>
        </div>
      </div>
