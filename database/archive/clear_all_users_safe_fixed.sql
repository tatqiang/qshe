...
-- Find and reset sequences automatically
-- Reset users sequence (find the actual name)
SELECT 'Resetting sequences...' as status;

-- Reset sequences for tables that have identity columns
-- This works regardless of the actual sequence names
DO $$
DECLARE
    seq_name text;
BEGIN
    ...
    FOR seq_name IN 
        ...
        WHERE pg_get_serial_sequence('public.users', 'id') IS NOT NULL
    LOOP
        EXECUTE format('ALTER SEQUENCE %s RESTART WITH 1', seq_name);
    END LOOP;
    
    -- Reset projects table sequence if it exists
    FOR seq_name IN 
        SELECT pg_get_serial_sequence('public.projects', 'id')
        WHERE pg_get_serial_sequence('public.projects', 'id') IS NOT NULL
    LOOP
        EXECUTE format('ALTER SEQUENCE %s RESTART WITH 1', seq_name);
    END LOOP;
    
    -- Reset main_areas table sequence if it exists
    FOR seq_name IN 
        SELECT pg_get_serial_sequence('public.main_areas', 'id')
        WHERE pg_get_serial_sequence('public.main_areas', 'id') IS NOT NULL
    LOOP
        EXECUTE format('ALTER SEQUENCE %s RESTART WITH 1', seq_name);
    END LOOP;
END $$;
